...
do
	...
	echo "" >> $FILE
	echo "model = 0         * Models for ω varying across lineages" >> $FILE
	echo "NSsites = 0          * Models for ω varying across sites" >> $FILE
	...
	echo "fix_omega = 0         * Estimate or fix omega" >> $FILE
	echo "omega = 0.5        * Initial or fixed omega" >> $FILE
done
